...
from prelude import make_base_dir, closest_encounter, distance

sta_f = open('all_station_crossing_db_updated111.txt','r')
...
for line in sta_f.readlines():
    ...
    for st in range(len(seismo_data)):
        if sta[st] == station:
            sta_lat = seismo_latitudes[st]
            sta_lon = seismo_longitudes[st]
            dist = distance(clat, clon, sta_lat, sta_lon)
            break
        else:
            continue
    ...
    axs[1].scatter(sta_lon, sta_lat, c='#e41a1c', s=50, zorder=3)

    axs[1].text(xx,yy, str(round(dist, 3))+' km', fontsize=15, fontweight='bold')
