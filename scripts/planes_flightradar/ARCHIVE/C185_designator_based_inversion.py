...
output = open('output/'+air_des+'_data', 'a+') #read and append correction

# Loop through each station in text file that we already know comes within 2km of the nodes
 
for line in sta_f.readlines():
    if air_des not in line:
        continue
    val = line.split(',')
    date = val[0]
    flight = val[1]
    ...
    #spec_dir = '/scratch/irseppi/nodal_data/plane_info/inversepicks/'+str(date[0:4])+'-'+str(date[4:6])+'-'+str(date[6:8])+'/'+str(flight)+'/'+str(sta)+'/'
    ...
    if Path(output1).exists():
        ...
        if len(coords) > 0:
            ...
            plt.scatter(coords_array[:,0], coords_array[:,1], color='black', marker='x')  
            plt.show()
            pick_again = input("Do you want to repick? (y or n)")
        else:
            continue
    else:
    ...
    mid = (coords_array[0,1]+coords_array[-1,1])/2
    diff = np.abs(coords_array[:,1] - mid)
    middle_p = np.argmin(diff)

    tprime0 = coords_array[middle_p,0]
    tprime = tprime0
    f0 = calc_f0(tprime, tprime0, coords_array[middle_p,1], v0, l, c)
    c = 343
    m0 = [f0, v0, l, tprime0]
    ...
    if isinstance(sta, int):
        ...
        for t_f in range(len(times)):
                try:      
                    ...
                    comp_freq.append(max_amplitude_frequency)
        ...
        delf = np.array(ft) - np.array(comp_freq)
    ...
    if Path(output2).exists():
        ...
        if len(peaks) > 0:
            ...
            freqpeak = np.array(freqpeak)
            plt.figure()
            plt.pcolormesh(times, frequencies, spec, shading='gouraud', cmap='pink_r', vmin=vmin, vmax=vmax)
            
            plt.scatter(freqpeak, peaks, color='black', marker='x')  
            plt.show()
            pick_again = input("Do you want to repick? (y or n)")
        else:
            pick_again = 'y'
    ...
    con = input("Do you want to invert this? (y or n)")
    if con == 'n':
        continue
    else:
        corridor_width = 6 
        ...
        if time_pick == True:
            ...
            if Path(output3).exists():
                ...
                plt.axvline(set_time[0], color='red', linestyle='--', linewidth=0.7) 
                plt.axvline(set_time[1], color='red', linestyle='--', linewidth=0.7)
                plt.show()
                pick_again = input("Do you want to repick? (y or n)")
            else:
            ...
            while pick_again == 'y':
                ...
                def onclick(event):
                    ...
                    plt.axvline(event.xdata, color='red', linestyle='--', linewidth=0.7) 
                    plt.draw() 
                    print('Clicked:', event.xdata)  
                    r3.write(str(event.xdata)+',\n')
        ...
        for pp in range(len(f0_array)):
            ...
            f0lab.append(str(np.round(f0,2))+',')
            f0write.append(str(f0))

            covlab.append(str(np.round(covm[pp],2))+',')
            covwrite.append(str(covm[pp]))
        fss = 'x-small'
        ...
        plt.show()     
        qnum = input('What quality number would you give this?(first num for signal quality (0 = none, 1 = weak, 2 = good, 3 = exceptional), second for ability to fit the model to data(0 = not appropriate, 1 = kind off appropriate, 2 = appropriate))')

        BASE_DIR = '/scratch/irseppi/nodal_data/plane_info/5plane_spec/2019-0'+str(month)+'-'+str(day)+'/'+str(flight)+'/'+str(sta)+'/'
        make_base_dir(BASE_DIR)
        fig.savefig('/scratch/irseppi/nodal_data/plane_info/5plane_spec/2019-0'+str(month)+'-'+str(day)+'/'+str(flight)+'/'+str(sta)+'/'+str(tlab)+'_'+str(flight)+'_Q'+str(qnum)+'.png')
        plt.close()
        ...
        output.write(str(date)+','+str(flight)+','+str(sta)+','+str(tm)+','+str(tprime0)+','+str(v0)+','+str(l)+',f0:,'+','.join(f0write)+',cov:,'+','.join(covwrite)+',Q'+str(qnum)+',\n') 
